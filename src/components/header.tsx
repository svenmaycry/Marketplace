import logoImage from '../assets/images/obsidian.svg';
import cardIcon from '../assets/images/bx-cart.svg';
...
const Header = () => {
	...
	return (
		...
		>
			<img className=' py-3' src={logoImage} alt="" width='250' />

			<button className='bg-transparent border-none' onClick={() => setIsShowCard(!isShowCard)}>
				<img src={cardIcon} alt="" width='40' />
			</button>

			<div className={cn('absolute right-0 top-20 shadow-lg p-5 rounded-lg mr-2', {
				hidden: !isShowCard
			})}
			>
				{cardItems.map(item => (
					<div className='flex items-center mb-1' key={`cart item ${item.name}`}>
						<img
							src={item.imagePath}
							alt={item.name}
							width='55'
